...
label_names=label_names([1:3,10:13]);

temporallh=label_names(1:3);
temporalrh=label_names(4:7);


tempalllh=alllabel(1:9);
tempallrh=alllabel(10:18);
...
for isubj=1:8
    ...
    S=squeeze(mean(sol,3));
      ...
      stem=[sim_dir 'source_data_subj_' num2str(isubj) '_cond_' num2str(icond) '_' tag];
    tmin=time(1);
    tstep=1/fs;
    ...
    for iLabel=1:length(alllabel)
        ...
        save([sim_dir 'simulation_subj_' num2str(isubj) 'epochs_' alllabel{iLabel}(1:end-6) '_cond' num2str(icond) '_' tag '.mat'], ...
    ...
    do_coherence(tempalllh,tempallrh,time,isubj,freq,icond,sim_dir, noiseLevelr,jitter_noise,tag)
