...
label_names=label_names([1:3,22:25]);
...
%% in v8 we have templh-temprh sources 
raw=fiff_setup_read_raw('/autofs/cluster/transcend/MEG/erm/038301/1/038301_erm_1_raw.fif');
...
%     flagSignal=0;
%     parfor iLabel=4:length(temporallh)
%         iLabel
...
%     parfor iLabel=length(temporallh)+1:length(srcind)-4
%         
%         tt{iLabel}=genLabelData(freq,length(srcind{iLabel}),nEpochs,jitter,time,noiseLevel,varAcrossFreq,flagSignal);
%         
%     end
    ...
    do_coherence(temporallh,frontal,time,isubj,freq,icond,sim_dir, noiseLevelr,jitter_noise)
