...
%%
raw=fiff_setup_read_raw('/autofs/cluster/transcend/MEG/erm/038301/1/038301_erm_1_sss.fif');
...
for isubj=1:8
    ...
    tt=cell(1);
    tic
    parfor iLabel=length(occipital)+1:length(srcind)
        
        tt{iLabel}=genLabelData(freq,length(srcind{iLabel}),nEpochs,jitter,time,noiseLevel,varAcrossFreq);
        
    end
    for iLabel=length(occipital)+1:length(srcind)
        cortex(srcind{iLabel},:,:)=tt{iLabel};
    end
    toc
    ...
    for iLabel=1:length(label_names)
        ...
        save([sim_dir 'sim_subj_' num2str(isubj) 'epochs_' label_names{iLabel}(1:end-6) '_cond_' num2str(icond) '_erm' num2str(noiseLevelr) ...
            , 'jittern'  num2str(jitter_noise(1,1)) '_' num2str(jitter_noise(1,2)) '_' num2str(jitter_noise(2,1)) '_' num2str(jitter_noise(2,2)) '.mat'], ...
    ...
    do_coherence_erm(occipital,frontal,time,isubj,freq,icond,sim_dir, noiseLevelr,jitter_noise)
